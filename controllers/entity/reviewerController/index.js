...
const { arrangeReviewerList } = require('@/utils/dataStructure');
...
class ReviewerController {
  ...
  static async findAll(req, res) {
    try {
      ...
      const [rows] = await pool.query(`SELECT * FROM review_main WHERE revlist_id = ?; SELECT * FROM reviewer_list WHERE rev_id = ?`, [rev_id, rev_id]);
      const reviwersList = arrangeReviewerList(rows[1])

      return res.status(200).json({
        status: true,
        data: {
          title: rows[0][0].title,
          postedOn: rows[0][0].postedOn,
          description: rows[0][0].content,
          reviewers: reviwersList
        }
      });
    } catch (error) {
